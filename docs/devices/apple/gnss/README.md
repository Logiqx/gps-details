...
Towards the end of this document is a list of different watch models and the GNSS chipsets. A distinction has been made between GNSS chipsets being confirmed (e.g. photographic evidence) or where it is speculated. Speculations may well be correct, due to unique characteristics in the data. 
...
The GNSS chipset often cannot be determined by simply disassembling a watch. The components on the SiP are typically encased inside resin which is a process known as [potting](https://en.wikipedia.org/wiki/Potting_(electronics)). Companies such as [TechInsights](https://www.techinsights.com/) are capable of [decapping](https://en.wikipedia.org/wiki/Decapping) the SiP and revealing the individual components, but they charge around 8000 EUR for their detailed reports for specific watch models.

The GNSS chipset inside Apple watches are always mounted on the SiP. Sometimes the GNSS chipset is on the the outside of the SiP module, but often it is on the inside and thus encased in resin. If the GNSS chipset is on the inside of the SiP it will often have the original part number printed on it, but when on the outside of the SiP it only has an anonymous [data matrix code](https://en.wikipedia.org/wiki/Data_Matrix).
...
The Apple Watch Series 3 introduced cellular capabilities and so there were essentially two models; Apple Watch Series 3 GPS + Cellular and Apple Watch Series 3 GPS. These pages will simply refer to cellular and non-cellular models.

Cellular and non-cellular models have a different SiP (and different antennas) which can clearly be seen in the teardowns; e.g. Apple Watch [Series 3](series-3.md) and [Series 6](series-6.md). The motherboard designs are always different because of the additional cellular components.
...
- BCM47754 (S5 + S6)
  - BCM47754KUBG was only mentioned in a [product brief](https://docs.broadcom.com/doc/12379501) but without any further details
- BCM47764 (S7)
...
It is worth mentioning that Synaptics acquired the rights to several Broadcom GNSS chipsets but marketed as the SYN477X2, SYN477X5, SYN477X8. These chipsets are available from sites such as [EDOM Technology](https://www.edomtech.com/en/product-detail/synaptics-gps-gnss-solutions/), [Macnica](https://www.macnica.co.jp/en/business/semiconductor/manufacturers/synaptics/products/143404/), and [Marubun](https://www.marubun.co.jp/products/20290/) but the absence of a SYN47754 and SYN47764 is noticeable.

It seems quite likely the Broadcom BCM477x4 chipsets are Apple specific, and primarily used in the various watches.
...
Click the watch links for detailed information about the individual models, investigations, photos, etc.
