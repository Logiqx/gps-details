...
The fix in V3.0506.0 appeared to remove the disparity of the issue in V3.0408.0, but additional filtering / smoothing was clearly apparent:

![apparent fix](img\apparent-fix.png)

The nature of the fix in V3.0506.0 / V3.0508.0 was not evident until issues appeared in Aug 2024, affecting the VERTIX and APEX Pro:

![9 mins](../smoothing/img/apex-pro-03-0508.png)

Comparing the APEX Pro speeds (magenta) against our two benchmark devices (u-blox M10 chipset, red and blue) shows the new data quality issue:
...
The changes in V3.0506.0 / V3.0508.0 are thought to be the same for the earlier watches (APEX Pro and VERTIX) and newer watches (APEX 2, APEX 2 Pro, VERTIX 2, VERTIX 2S). It's hard to be 100% sure that the changes were applied universal, but it seems relatively likely.

Since the original problem did not affect watches using the Sony GNSS chipset, the dramatic changes in the VERTIX and APEX Pro data seem likely to be caused by positional data being used for speeds, instead of the speed calculated by the Sony GNSS chipset.

This is particularly bad for the COROS watches using the Sony CXD56xxxx, but also bad for the newer models using the Airoha AG3335M. The fix that we were expecting was to tweak some parameters for the AG3335M, not a change which (apparently) discards the speed data from the GNSS chipset.
...
The windsurfing / speedsailing community need the same quality speedsurfing data (or better) as we had back in April 2024. We believe the best way to achieve this goal is to identify and fix the root cause of the issue found in May 2024.
...
Airoha chipsets have a number of power saving modes:
...
Power saving options are attractive for sports such as ultra-running and ultra-endurance cycling but they are not necessary for windsurfing / speedsurfing. Power saving comes at the expense of data quality which in most sports is not hugely important, but for a sport gauging performance on instantaneous top speeds, power saving is highly undesirable.

Power saving could be one of the reasons for the issues seen on the APEX 2 Pro, VERTIX 2 and VERTIX 2S with V3.0408.0. Windsurfing and speedsurfing modes should switch off all power saving options, regardless of whether they caused the issue in V3.0408.0. Power saving will inevitably lead to spikes, which seem particularly common on COROS watches using regular GPS mode (e.g. 50 knots instead of 20 knots).
...
Corresponding commands are also available to check these settings.
...
This is well understood (standard sampling theory), but it is interesting to note that the Forerunner 255 (red) exhibits the same aliasing as the 1 Hz u-blox chip (blue). The Forerunner 255 is using exactly the same AG3335M chip as the COROS watches.

![apparent fix](img\walking-fr-255.png)

We can only speculate how Airoha have implemented down-sampling within the AG3335M, but it is conceivable that changing the fix rate may trigger other changes, such as the dynamic models and filters being applied.
...
If the issue in V3.0408.0 cannot be remedied by disabling power saving (or selecting a different activity mode) then it is worth experimenting with these two parameters, imho.
...
Speedsurfing mode should be recording the Sony / Airoha data without an additional filtering / smoothing applied. It is particularly important that the speeds calculated by the GNSS chipsets (doppler-derived) are unfiltered. The windsurfing mode should really handle the GNSS data in exactly the same way as speedsurfing, because they are the same community wanting to know the same thing - accurate speeds.

The requirement of the speedsurfing community is to regain the data quality that we had in April, so that we can all compete on a level playing field. To achieve this there are two activities:

1) Roll back the speedsurfing code to behave like it was in V3.0408.0.
2) Address the underlying issue relating to the Airoha data in V3.0408.0.

Fixing the root cause must surely be possible, because Garmin faced exactly the same issue (over 2 years ago) and have subsequently resolved it.

Hopefully the information in this document will prove to be helpful in diagnosing and fixing the current issue(s).
